...
class WebsocketBase:
    ...
    async def _receive_messages(self) -> None:
        log.info(f"{self._name} starting to receive messages")
        try:
            log.info(
            ...
            while (
                ...
                log.info(f"{self._name} ws_msg={ws_msg}")
                if ws_msg.type == WSMsgType.BINARY:
                    ...
                    log.info(f"{self._name} recieved error message")
                    break
                elif ws_msg.type == WSMsgType.CLOSED:
                    log.info(
                        f"{self._name} recieved closed message, data={ws_msg.data}"
                    )
                    break
                    ...
                    log.info(
                        f"{self._name} recieved closing message, data={ws_msg.data}"
                    )
                    break
                # TODO: handle all other message types
        except asyncio.CancelledError:
            log.info(f"{self._name}: {self._receive_messages.__name__} cancelled")
            raise
